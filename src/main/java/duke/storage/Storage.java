...
public class Storage {
     ...
     * @return Task subclass instance (either duke.Todo, Deadline or duke.Event)
     */
        ...
        if (taskType == Todo.class) {
            ...
            if (tokens.length != 4) {
                throw new MissingTokenParseError(String.format(
                        "duke.model.Deadline task expected 4 values, received %d instead.",
                        tokens.length
                ));
            }
            ...
            if (tokens.length != 4) {
                throw new MissingTokenParseError(String.format(
                        "duke.model.Event task expected 4 values, received %d instead.",
                        tokens.length
                ));
            }
    ...
    /**
     * Parses the textual representation of a Task subtype (e.g. "T" returns duke.Todo.class).
     *
