...
1. Open the `build.gradle` file in an editor. Update the following code block to point to the main class (i.e., the one containing the `main` method) of your application. The code below assumes your main class is `seedu.seedu.duke.Duke`
   ```groovy
   application {
       mainClassName = "seedu.seedu.duke.Duke"
   }
   ```
...
```groovy
shadowJar {
    archiveBaseName = sseedu.duke.duke
...
├─main
│  └─java
│     └─seseedu.dukeduke/Duke.java
└─test
    └─java
        └─seseedu.dukeduke/DukeTest.java
```
...
You can now write a test (e.g., `test/java/seseedu.dukeduke/DukeTest.java`) and run it with `gradlew test`.
