...
Controls are reusable UserInterface elements. Refer to the [JavaFX's official documentation](https://openjfx.io/javadoc/11/javafx.controls/javafx/scene/control/package-summary.html) for a list of controls available.
...
Now that we know what controls we need to implement our UserInterface, let’s start programming! We quickly run into a problem: how do we show all of them on the screen at once? 
...
But how do we get the exact layout we want in the UserInterface? JavaFX provides that functionality in the form of **layout panes** in `javafx.scene.layouts`. Each layout pane follows a _layout policy_ to decide how to arrange its children. For example, the `VBox` lays out its children in a single vertical column and its counterpart, the `HBox` lays out its children in a single horizontal row. 
