...
Let’s create our custom control `controllers.DialogBox`:
```java
...
public class controllers.DialogBox extends HBox {
    ...
    public controllers.DialogBox(Label l, ImageView iv) {
        text = l;
        displayPicture = iv;
...
private void handleUserInput() {
    ...
    dialogContainer.getChildren().addAll(
            new controllers.DialogBox(userText, new ImageView(user)),
            new controllers.DialogBox(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
...
## Iteration 3 – Adding custom behavior to controllers.DialogBox

One additional benefit of defining a custom control is that we can add behavior specific to our `controllers.DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and duke.Duke’s output.
...
public static controllers.DialogBox getUserDialog(Label l, ImageView iv) {
    return new controllers.DialogBox(l, iv);
}

public static controllers.DialogBox getDukeDialog(Label l, ImageView iv) {
    var db = new controllers.DialogBox(l, iv);
    db.flip();
    return db;
}
```
...
Now, we can go back to the `Main` class and change the event handler to use our new `controllers.DialogBox`. 
...
private void handleUserInput() {
    ...
    Label dukeText = new Label(getResponse(userInput.getText()));
    dialogContainer.getChildren().addAll(
            controllers.DialogBox.getUserDialog(userText, new ImageView(user)),
            controllers.DialogBox.getDukeDialog(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
```
...
1. While the GUI looks similar to the mockup, there are still parts that need to be refined. Try your hand at some of these tasks:
   * Add padding between each controllers.DialogBox
