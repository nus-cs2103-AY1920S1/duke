...
For now, let’s have the application add a new `Label` with the text from the `TextField`. Update the `seedu.duke.Main` class as follows. You'll need to add an `import javafx.scene.control.Label;` too.
```java
@Override
...
We use the code in our main class just like any other control. Here are the steps to update the code to use the custom control in `seedu.duke.Main.java`.
...
Now, we can go back to the `seedu.duke.Main` class and change the event handler to use our new `DialogBox`. 
