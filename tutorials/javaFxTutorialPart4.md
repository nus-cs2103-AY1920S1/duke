...
1. Components are heavily dependent on each other:
   Why does `Main` need to know that `duke.main.DialogBox` needs a `Label`? 
...
public class duke.main.DialogBox extends HBox {
    ...
    private duke.main.DialogBox(String text, Image img) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(MainWindow.class.getResource("/view/duke.main.DialogBox.fxml"));
