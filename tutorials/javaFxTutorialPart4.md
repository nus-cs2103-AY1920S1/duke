...
As part of the effort to separate the code handling duke.Duke's logic and duke.execution.UI, let's _refactor_ the duke.execution.UI-related code to its own class.
We call these duke.execution.UI classes _controllers_.
...
Without the annotation, we will have to make everything `public` and expose our duke.execution.UI to unwanted changes.
...
Let's create a new `duke.Main` class as the bridge between the existing logic in `duke.Duke` and the duke.execution.UI in `duke.MainWindow`.
