...
**duke.ui.controllers.MainWindow.fxml**
```xml
...
<AnchorPane maxHeight="-Infinity" maxWidth="-Infinity" minHeight="-Infinity" minWidth="-Infinity" prefHeight="600.0" prefWidth="400.0" xmlns="http://javafx.com/javafx/8.0.171" xmlns:fx="http://javafx.com/fxml/1" fx:controller="duke.ui.controllers.MainWindow">
   <children>
      <TextField fx:id="userInput" layoutY="558.0" onAction="#handleUserInput" prefHeight="41.0" prefWidth="324.0" AnchorPane.bottomAnchor="1.0" />
...
1. Let’s explore the provided FXML files in Scene Builder. 
    ...
    Select `Open Project` > `src/main/resources/view/duke.ui.controllers.MainWindow.fxml`. Inspect each control and its properties.

   ![SceneBuilder opening duke.ui.controllers.MainWindow.fxml](assets/SceneBuilder.png)
...
1. On the left accordion, you can see that we have set the controller class to `duke.ui.controllers.MainWindow`. 
We will get to that later.
 
   ![Controller for duke.ui.controllers.MainWindow](assets/MainWindowController.png)
...
Let's implement the `duke.ui.controllers.MainWindow` controller class that we specified in `duke.ui.controllers.MainWindow.fxml`.

**duke.ui.controllers.MainWindow.java**
```java
...
import javafx.scene.layout.VBox;
/**
 * Controller for duke.ui.controllers.MainWindow. Provides the layout for the other controls.
 */
public class duke.ui.controllers.MainWindow extends AnchorPane {
    @FXML
...
Notice how in `duke.ui.controllers.MainWindow`, we can invoke `TextField#clear()` on `userInput` and access its content just as we did in the previous example.
...
Let's create a new `Main` class as the bridge between the existing logic in `duke.Duke` and the UI in `duke.ui.controllers.MainWindow`.
...
public class Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(Main.class.getResource("/view/duke.ui.controllers.MainWindow.fxml"));
            AnchorPane ap = fxmlLoader.load();
            Scene scene = new Scene(ap);
            stage.setScene(scene);
            fxmlLoader.<duke.ui.controllers.MainWindow>getController().setDuke(duke);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
public class DialogBox extends HBox {
    ...
    private DialogBox(String text, Image img) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(duke.ui.controllers.MainWindow.class.getResource("/view/DialogBox.fxml"));
...
1. Convert `duke.ui.controllers.MainWindow` to use the `fx:root` construct.
1. Extend `duke.ui.controllers.MainWindow` to have a `Stage` as a root Node.
