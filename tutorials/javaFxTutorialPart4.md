...
1. Components are heavily dependent on each other:
   Why does `Main` need to know that `javafx.DialogBox` needs a `Label`? 
...
Let's create a new `Main` class as the bridge between the existing logic in `Duke` and the UI in `javafx.MainWindow`.
...
public class Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(Main.class.getResource("/view/javafx.MainWindow.fxml"));
