...
<AnchorPane maxHeight="-Infinity" maxWidth="-Infinity" minHeight="-Infinity" minWidth="-Infinity" prefHeight="600.0" prefWidth="400.0" xmlns="http://javafx.com/javafx/8.0.171" xmlns:fx="http://javafx.com/fxml/1" fx:controller="duke.command.MainWindow">
   <children>
      <TextField fx:id="userInput" layoutY="558.0" onAction="#handleUserInput" prefHeight="41.0" prefWidth="324.0" AnchorPane.bottomAnchor="1.0" />
...
1. Let’s explore the provided FXML files in Scene Builder. 
    ...
    Select `Open Project` > `src/main/resources/view/duke.command.MainWindow.fxml`. Inspect each control and its properties.

   ![SceneBuilder opening duke.command.MainWindow.fxml](assets/SceneBuilder.png)
...
1. On the left accordion, you can see that we have set the controller class to `duke.command.MainWindow`. 
We will get to that later.
 
   ![Controller for duke.command.MainWindow](assets/MainWindowController.png)
...
Let's implement the `duke.command.MainWindow` controller class that we specified in `duke.command.MainWindow.fxml`.

**duke.command.MainWindow.java**
```java
 ...
 */
public class duke.command.MainWindow extends AnchorPane {
    @FXML
...
Notice how in `duke.command.MainWindow`, we can invoke `TextField#clear()` on `userInput` and access its content just as we did in the previous example.
...
Let's create a new `Main` class as the bridge between the existing logic in `Duke` and the UI in `duke.command.MainWindow`.
...
public class Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(Main.class.getResource("/view/duke.command.MainWindow.fxml"));
            AnchorPane ap = fxmlLoader.load();
            Scene scene = new Scene(ap);
            stage.setScene(scene);
            fxmlLoader.<duke.command.MainWindow>getController().setDuke(duke);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
public class DialogBox extends HBox {
    ...
    private DialogBox(String text, Image img) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(duke.command.MainWindow.class.getResource("/view/DialogBox.fxml"));
...
1. Convert `duke.command.MainWindow` to use the `fx:root` construct.
1. Extend `duke.command.MainWindow` to have a `Stage` as a root Node.
