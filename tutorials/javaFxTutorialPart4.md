...
As part of the effort to separate the code handling Duke's logic and UserInterface, let's _refactor_ the UserInterface-related code to its own class.
We call these UserInterface classes _controllers_. 
...
Without the annotation, we will have to make everything `public` and expose our UserInterface to unwanted changes.
...
Let's create a new `Main` class as the bridge between the existing logic in `Duke` and the UserInterface in `MainWindow`.
