...
1. Components are heavily dependent on each other:
   Why does `duke.view.Main` need to know that `DialogBox` needs a `Label`? 
...
1. The code is untidy and long:
   ...
   The `duke.view.Main` class attempts to do it all. 
...
Let's create a new `duke.view.Main` class as the bridge between the existing logic in `Duke` and the UI in `MainWindow`.
 ...
 */
public class duke.view.Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(duke.view.Main.class.getResource("/view/MainWindow.fxml"));
...
```java
//...    
Application.launch(duke.view.Main.class, args);
//...
```
