...
As part of the effort to separate the code handling Duke's logic and execution.UI, let's _refactor_ the execution.UI-related code to its own class.
We call these execution.UI classes _controllers_.
...
Without the annotation, we will have to make everything `public` and expose our execution.UI to unwanted changes.
...
Let's create a new `Main` class as the bridge between the existing logic in `Duke` and the execution.UI in `MainWindow`.
