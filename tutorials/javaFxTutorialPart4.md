...
Let's return to IceBear and convert it to use FXML instead.
...
As part of the effort to separate the code handling IceBear's logic and UI, let's _refactor_ the UI-related code to its own class.
...
Let's create a new `Main` class as the bridge between the existing logic in `IceBear` and the UI in `MainWindow`.
...
/**
 * A GUI for IceBear using FXML.
 */
public class Main extends Application {

    private IceBear duke = new IceBear();
