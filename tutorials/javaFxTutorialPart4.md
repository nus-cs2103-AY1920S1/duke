...
<AnchorPane maxHeight="-Infinity" maxWidth="-Infinity" minHeight="-Infinity" minWidth="-Infinity" prefHeight="600.0" prefWidth="400.0" xmlns="http://javafx.com/javafx/8.0.171" xmlns:fx="http://javafx.com/fxml/1" fx:controller="seedu.duke.ui.MainWindow">
   <children>
      <TextField fx:id="userInput" layoutY="558.0" onAction="#handleUserInput" prefHeight="41.0" prefWidth="324.0" AnchorPane.bottomAnchor="1.0" />
...
1. Let’s explore the provided FXML files in Scene Builder. 
    ...
    Select `Open Project` > `src/main/resources/view/seedu.duke.ui.MainWindow.fxml`. Inspect each control and its properties.

   ![SceneBuilder opening seedu.duke.ui.MainWindow.fxml](assets/SceneBuilder.png)
...
1. On the left accordion, you can see that we have set the controller class to `seedu.duke.ui.MainWindow`. 
We will get to that later.
 
   ![Controller for seedu.duke.ui.MainWindow](assets/MainWindowController.png)
...
Let's implement the `seedu.duke.ui.MainWindow` controller class that we specified in `seedu.duke.ui.MainWindow.fxml`.

**seedu.duke.ui.MainWindow.java**
```java
 ...
 */
public class seedu.duke.ui.MainWindow extends AnchorPane {
    @FXML
...
Notice how in `seedu.duke.ui.MainWindow`, we can invoke `TextField#clear()` on `userInput` and access its content just as we did in the previous example.
...
Let's create a new `Main` class as the bridge between the existing logic in `Duke` and the UI in `seedu.duke.ui.MainWindow`.
...
public class Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(Main.class.getResource("/view/seedu.duke.ui.MainWindow.fxml"));
            AnchorPane ap = fxmlLoader.load();
            Scene scene = new Scene(ap);
            stage.setScene(scene);
            fxmlLoader.<seedu.duke.ui.MainWindow>getController().setDuke(duke);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
public class DialogBox extends HBox {
    ...
    private DialogBox(String text, Image img) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(seedu.duke.ui.MainWindow.class.getResource("/view/DialogBox.fxml"));
...
The last change that we have to make is to point our `seedu.duke.core.Launcher` class in the right direction:
In `seedu.duke.core.Launcher.java`
```java
//...    
...
1. Convert `seedu.duke.ui.MainWindow` to use the `fx:root` construct.
1. Extend `seedu.duke.ui.MainWindow` to have a `Stage` as a root Node.
