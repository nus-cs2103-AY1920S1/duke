...
Let's return to duke.util.Duke and convert it to use FXML instead.
...
**duke.javafx.control.MainWindow.fxml**
```xml
...
<AnchorPane maxHeight="-Infinity" maxWidth="-Infinity" minHeight="-Infinity" minWidth="-Infinity" prefHeight="600.0" prefWidth="400.0" xmlns="http://javafx.com/javafx/8.0.171" xmlns:fx="http://javafx.com/fxml/1" fx:controller="duke.javafx.control.MainWindow">
   <children>
      <TextField fx:id="userInput" layoutY="558.0" onAction="#handleUserInput" prefHeight="41.0" prefWidth="324.0" AnchorPane.bottomAnchor="1.0" />
...
1. Let’s explore the provided FXML files in Scene Builder. 
    ...
    Select `Open Project` > `src/main/resources/view/duke.javafx.control.MainWindow.fxml`. Inspect each control and its properties.

   ![SceneBuilder opening duke.javafx.control.MainWindow.fxml](assets/SceneBuilder.png)
...
1. On the left accordion, you can see that we have set the controller class to `duke.javafx.control.MainWindow`. 
We will get to that later.
 
   ![Controller for duke.javafx.control.MainWindow](assets/MainWindowController.png)
...
As part of the effort to separate the code handling duke.util.Duke's logic and UI, let's _refactor_ the UI-related code to its own class.
We call these UI classes _controllers_. 

Let's implement the `duke.javafx.control.MainWindow` controller class that we specified in `duke.javafx.control.MainWindow.fxml`.

**duke.javafx.control.MainWindow.java**
```java
...
import javafx.scene.layout.VBox;
/**
 * Controller for duke.javafx.control.MainWindow. Provides the layout for the other controls.
 */
public class duke.javafx.control.MainWindow extends AnchorPane {
    @FXML
    ...
    public void setDuke(duke.util.Duke d) {
        duke = d;
    }
...
Notice how in `duke.javafx.control.MainWindow`, we can invoke `TextField#clear()` on `userInput` and access its content just as we did in the previous example.
...
Let's create a new `Main` class as the bridge between the existing logic in `duke.util.Duke` and the UI in `duke.javafx.control.MainWindow`.
...
/**
 * A GUI for duke.util.Duke using FXML.
 */
public class Main extends Application {

    private duke.util.Duke duke = new duke.util.Duke();

    @Override
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(Main.class.getResource("/view/duke.javafx.control.MainWindow.fxml"));
            AnchorPane ap = fxmlLoader.load();
            Scene scene = new Scene(ap);
            stage.setScene(scene);
            fxmlLoader.<duke.javafx.control.MainWindow>getController().setDuke(duke);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
public class DialogBox extends HBox {
    ...
    private DialogBox(String text, Image img) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(duke.javafx.control.MainWindow.class.getResource("/view/DialogBox.fxml"));
...
1. Convert `duke.javafx.control.MainWindow` to use the `fx:root` construct.
1. Extend `duke.javafx.control.MainWindow` to have a `Stage` as a root Node.
