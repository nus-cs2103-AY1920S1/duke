...
1. Components are heavily dependent on each other:
   Why does `seedu.duke.Main` need to know that `DialogBox` needs a `Label`? 
...
1. The code is untidy and long:
   ...
   The `seedu.duke.Main` class attempts to do it all. 
...
Let's create a new `seedu.duke.Main` class as the bridge between the existing logic in `Duke` and the UI in `MainWindow`.

**seedu.duke.Main.java**
```java
@Override
 ...
 */
public class seedu.duke.Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(seedu.duke.Main.class.getResource("/view/MainWindow.fxml"));
...
```java
//...    
Application.launch(seedu.duke.Main.class, args);
//...
```
