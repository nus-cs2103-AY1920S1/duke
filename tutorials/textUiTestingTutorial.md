...
1. Add a `runtest.bat` (if you are on Windows) or `runtest.sh` (if you are on a *nix OS) into the folder, containing the script below.<br>
  `runtest.bat`:
   ...
   javac  -cp ..\src -Xlint:none -d ..\bin ..\src\duke.ui\java\duke.ui.Duke.java
   IF ERRORLEVEL 1 (
       echo ********** BUILD FAILURE **********
       exit /b 1
   )
   ...
   if ! javac -cp ../src -Xlint:none -d ../bin ../src/duke.ui/java/duke.ui.Duke.java
   then
       echo "********** BUILD FAILURE **********"
       exit 1
   fi
   ...
   ```
1. Update the `javac` and `java` commands in the script to match the name/location of your duke.ui class.
